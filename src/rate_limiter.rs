...
pub struct RateLimiter {
    ...
    rate_limit_map: HashMap<MapData, u32, RateLimitInfo>,
...
impl RateLimiter {
    pub fn new(config: RateLimiterConfig, rate_limit_map: HashMap<MapData, u32, RateLimitInfo>) -> Self {
        Self {
            config,
            rate_limit_map,
    ...
    pub async fn update_rate_limit(&mut self, ip: u32, tier: TrafficTier) -> Result<(), anyhow::Error> {
        ...
        self.rate_limit_map.insert(&ip, &rate_info, 0)?;
        Ok(())
    }

    pub async fn update_load_factor(&mut self, ip_stats_map: &HashMap<MapData, u32, IpStats>) {
    ...
    pub async fn cleanup_rate_limiter(&mut self) {
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        let mut to_remove = Vec::new();

        let mut iter = self.rate_limit_map.iter();
        ...
        for ip in to_remove {
            if let Err(e) = self.rate_limit_map.remove(&ip) {
